...
- Support for an exported Decision Tree or KNN gesture model
...
Open [`gesture_remote.ino`](firmware/gesture_remote/gesture_remote.ino), select the ESP32 board, compile, and upload. The included [`gesture_model.h`](firmware/gesture_remote/gesture_model.h) is a clearly marked placeholder. Replace it only if you already have a reviewed, compatible exported model header.
...
- The repository does not publish a fabricated dataset, trained header, training code, or accuracy value. Any model result must come from reviewed motion data and a separately validated training process.
