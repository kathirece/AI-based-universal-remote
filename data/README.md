...
5. Copy only the CSV header and data rows into `gesture_data.csv`. Lines beginning with `#` are comments and can remain because the notebook ignores them.
6. Train and evaluate with `notebooks/train_gesture_models.ipynb` in Google Colab.
